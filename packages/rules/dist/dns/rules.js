...
// =============================================================================
export const authoritativeFailureRule = {
    ...
    enabled: true,
    evaluate(context) {
        const failures = context.observations.filter((obs) => obs.vantageType === 'authoritative' &&
...
// =============================================================================
export const authoritativeMismatchRule = {
    ...
    evaluate(context) {
        ...
        if (inconsistentRecordSets.length === 0)
            return null;
        const results = [];
        for (const rs of inconsistentRecordSets) {
            // Only consider if authoritative vantages are involved
            const authObservations = context.observations.filter((obs) => obs.queryName.toLowerCase() === rs.name.toLowerCase() &&
                obs.queryType === rs.type &&
                obs.vantageType === 'authoritative' &&
                obs.status === 'success');
            if (authObservations.length < 2)
                continue;
...
// =============================================================================
export const recursiveAuthoritativeMismatchRule = {
    ...
    evaluate(context) {
        ...
        const recordSetsByQuery = new Map();
        ...
        for (const [key] of recordSetsByQuery) {
            ...
            const recursiveObs = context.observations.filter((obs) => obs.queryName.toLowerCase() === name.toLowerCase() &&
                ...
                obs.status === 'success');
            const authObs = context.observations.filter((obs) => obs.queryName.toLowerCase() === name.toLowerCase() &&
                obs.queryType === type &&
                obs.vantageType === 'authoritative' &&
                obs.status === 'success');
            if (recursiveObs.length === 0 || authObs.length === 0)
                continue;
...
// =============================================================================
export const cnameCoexistenceRule = {
    ...
    evaluate(context) {
        ...
        for (const cname of cnameRecords) {
            ...
            const conflicting = context.recordSets.filter((rs) => rs.name.toLowerCase() === cname.name.toLowerCase() &&
                rs.type !== 'CNAME' &&
...
// =============================================================================
export const unmanagedZonePartialCoverageRule = {
    ...
    evaluate(context) {
        ...
        if (context.zoneManagement !== 'unmanaged')
            return null;
...
// Helper Functions
// =============================================================================
function groupByQuery(observations) {
...
function extractUniqueValues(observations) {
