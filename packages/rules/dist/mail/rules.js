...
export const mxPresenceRule = {
    ...
    evaluate(context) {
        ...
        // Check for Null MX (0 .)
        const nullMxObs = successfulObs.find((obs) => obs.answerSection?.some((a) => /^0\s+\.$/.test(a.data.trim())));
        if (nullMxObs) {
            return {
                finding: {
        ...
        if (!hasMx) {
            ...
            const failures = mxObservations.filter((obs) => obs.status === 'timeout' || obs.status === 'error' || obs.status === 'refused');
...
export const spfRule = {
    ...
    evaluate(context) {
        ...
        const failedTxtObs = allTxtObservations.filter((obs) => obs.status === 'timeout' || obs.status === 'error' || obs.status === 'refused');
        // Look for SPF record in successful answers
        let spfRecord = null;
        let spfObservation = null;
...
export const dmarcRule = {
    ...
    evaluate(context) {
        ...
        const dmarcObservations = context.observations.filter((obs) => obs.queryType === 'TXT' &&
            obs.queryName.toLowerCase() === `_dmarc.${context.domainName}`.toLowerCase() &&
            obs.status === 'success');
        let dmarcRecord = null;
        let dmarcObservation = null;
...
export const mtaStsRule = {
    ...
    enabled: true,
    evaluate(context) {
        const mtaStsObservations = context.observations.filter((obs) => obs.queryType === 'TXT' &&
        ...
        const hasMtaSts = successfulObs.some((obs) => obs.answerSection?.some((a) => a.data.includes('v=STSv1')));
        if (!hasMtaSts) {
            return {
                finding: {
...
export const tlsRptRule = {
    ...
    enabled: true,
    evaluate(context) {
        const tlsRptObservations = context.observations.filter((obs) => obs.queryType === 'TXT' &&
        ...
        const hasTlsRpt = successfulObs.some((obs) => obs.answerSection?.some((a) => a.data.includes('v=TLSRPTv1')));
        if (!hasTlsRpt) {
            return {
                finding: {
...
export const bimiRule = {
    ...
    enabled: true,
    evaluate(context) {
        const bimiObservations = context.observations.filter((obs) => obs.queryType === 'TXT' &&
        ...
        const hasBimi = successfulObs.some((obs) => obs.answerSection?.some((a) => a.data.includes('v=BIMI1')));
        if (!hasBimi) {
