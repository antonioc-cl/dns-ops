...
// Type declarations for Web Crypto API (available in Node.js 18+ and Workers)
 ...
 */
export async function uuidv5(name: string, namespace: string = TENANT_NAMESPACE): Promise<string> {
 ...
 */
export async function normalizeTenantId(identifier: string): Promise<string> {
  if (!identifier) {
 ...
 * @returns Promise resolving to UUID string
 */
export async function getTenantUUID(identifier: string): Promise<string> {
...
export async function initTenantCache(): Promise<void> {
  ...
  await Promise.all(
    commonTenants.map(async (tenant) => {
