...
export const snapshots = pgTable('snapshots', {
    ...
    queriedNames: jsonb('queried_names').notNull().$type(),
    queriedTypes: jsonb('queried_types').notNull().$type(),
...
// VANTAGE POINT TABLE
// =============================================================================
export const vantagePoints = pgTable('vantage_points', {
    id: uuid('id').primaryKey().defaultRandom(),
    name: varchar('name', { length: 100 }).notNull(),
    ...
    endpoints: jsonb('endpoints').notNull().$type(),
...
export const observations = pgTable('observations', {
    ...
    additionalSection: jsonb('additional_section').$type(),
    // Error details
    errorMessage: text('error_message'),
...
export const recordSets = pgTable('record_sets', {
    ...
    values: jsonb('values').notNull().$type(),
    // Source observations that contributed to this record set
    sourceObservationIds: jsonb('source_observation_ids').notNull().$type(),
    sourceVantages: jsonb('source_vantages').notNull().$type(),
...
// =============================================================================
export const domainNotes = pgTable('domain_notes', {
    id: uuid('id').primaryKey().defaultRandom(),
    domainId: uuid('domain_id')
        .notNull()
        .references(() => domains.id, { onDelete: 'cascade' }),
...
export const templateOverrides = pgTable('template_overrides', {
    ...
    // Scope
    appliesToDomains: jsonb('applies_to_domains').$type(), // null = all domains
    // Actor context
    createdBy: varchar('created_by', { length: 100 }).notNull(),
    tenantId: uuid('tenant_id'),
    // Timestamps
    ...
    uniqueOverride: uniqueIndex('template_override_unique_idx').on(table.providerKey, table.templateKey, table.tenantId),
}));
// =============================================================================
