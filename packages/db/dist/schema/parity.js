 ...
 */
export const legacyAccessLogs = pgTable('legacy_access_logs', {
    id: uuid('id').primaryKey().defaultRandom(),
    ...
    responseTimeMs: jsonb('response_time_ms').$type(),
 ...
 */
export const providerBaselines = pgTable('provider_baselines', {
    id: uuid('id').primaryKey().defaultRandom(),
    ...
    dkimSelectors: jsonb('dkim_selectors').$type(),
    // MX patterns
    mxPatterns: jsonb('mx_patterns').$type(),
    // SPF includes
    spfIncludes: jsonb('spf_includes').$type(),
    // Documentation/notes
    notes: text('notes'),
 ...
 */
export const mismatchReports = pgTable('mismatch_reports', {
    id: uuid('id').primaryKey().defaultRandom(),
    // Report scope
    ...
    totalComparisons: jsonb('total_comparisons').$type().notNull(),
    matchCount: jsonb('match_count').$type().notNull(),
    mismatchCount: jsonb('mismatch_count').$type().notNull(),
    partialMatchCount: jsonb('partial_match_count').$type().notNull(),
    // Mismatch breakdown by field
    mismatchBreakdown: jsonb('mismatch_breakdown').$type(),
    // Adjudication summary
    adjudicatedCount: jsonb('adjudicated_count').$type(),
    pendingCount: jsonb('pending_count').$type(),
    // Cutover readiness
