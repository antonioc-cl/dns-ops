...
import { legacyAccessLogs, mismatchReports, providerBaselines, shadowComparisons, } from '../schema/parity.js';
// =============================================================================
...
export class ShadowComparisonRepository {
    ...
    async findByDomain(domain) {
        const results = await this.db.selectWhere(shadowComparisons, eq(shadowComparisons.domain, domain));
     ...
     */
    async findPendingAdjudications() {
     ...
     */
    async adjudicate(id, acknowledgedBy, adjudication, notes) {
        const existing = await this.findById(id);
        if (!existing)
            return undefined;
        const results = await this.db.update(shadowComparisons, {
...
export class LegacyAccessLogRepository {
     ...
     */
    async findByDomain(domain) {
        const results = await this.db.selectWhere(legacyAccessLogs, eq(legacyAccessLogs.domain, domain));
    ...
    async getStats() {
        ...
        const last24h = all.filter((l) => new Date(l.requestedAt) > new Date(Date.now() - 24 * 60 * 60 * 1000)).length;
        return {
            total: all.length,
            byToolType,
...
export class ProviderBaselineRepository {
    ...
    async update(id, data) {
        const results = await this.db.update(providerBaselines, { ...data, updatedAt: new Date() }, eq(providerBaselines.id, id));
        return results[0];
    }
    /**
     ...
     */
    async deprecate(id) {
        const results = await this.db.update(providerBaselines, { status: 'deprecated', updatedAt: new Date() }, eq(providerBaselines.id, id));
        return results[0];
    }
    /**
...
export class MismatchReportRepository {
     ...
     */
    async getLatestForDomain(domain) {
     ...
     */
    async generateReport(shadowComparisonRepo, domain, periodStart, periodEnd, generatedBy) {
        ...
        const pendingCount = periodComparisons.filter((c) => !c.adjudication && c.status !== 'match').length;
        ...
        for (const c of periodComparisons) {
            const fieldComparisons = c.comparisons;
            for (const fc of fieldComparisons) {
                if (fc.status === 'mismatch') {
                    const key = fc.field.replace(/-/g, '');
