 ...
 */
export const dkimSelectors = pgTable(
  'dkim_selectors',
  {
    ...
    hashAlgorithms: jsonb('hash_algorithms').$type<string[]>(), // sha256, sha1
    flags: jsonb('flags').$type<string[]>(), // t=s, t=y, etc.
 ...
 */
export const mailEvidence = pgTable(
  'mail_evidence',
  {
    id: uuid('id').primaryKey().defaultRandom(),
    snapshotId: uuid('snapshot_id')
    ...
    spfMechanisms: jsonb('spf_mechanisms').$type<string[]>(),
    ...
    dmarcRua: jsonb('dmarc_rua').$type<string[]>(), // Aggregate report URIs
    dmarcRuf: jsonb('dmarc_ruf').$type<string[]>(), // Forensic report URIs

    // DKIM summary
    hasDkim: boolean('has_dkim').notNull().default(false),
    dkimSelectorsFound: jsonb('dkim_selectors_found').$type<string[]>(),
    ...
    tlsRptRua: jsonb('tls_rpt_rua').$type<string[]>(),
