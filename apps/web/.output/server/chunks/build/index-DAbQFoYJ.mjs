import { n as ne, a as ac, d as dc, y as ye, o as oc, B as Bo, L as Lo } from '../nitro/nitro.mjs';
export { v as AlertRepository, e as AuditEventRepository, C as DkimSelectorRepository, J as DomainNoteRepository, D as DomainTagRepository, F as FindingRepository, Z as LegacyAccessLogRepository, _ as MailEvidenceRepository, N as MismatchReportRepository, b as MonitoredDomainRepository, j as ObservationRepository, l as ProviderBaselineRepository, k as RecordSetRepository, c as RemediationRepository, M as RulesetVersionRepository, f as SavedFilterRepository, t as ShadowComparisonRepository, g as SharedReportRepository, u as SimpleDatabaseAdapter, s as SnapshotRepository, I as SuggestionRepository, w as TemplateOverrideRepository, h as adjudicationEnum, i as alertStatusEnum, m as alerts, p as auditActionEnum, q as auditEvents, r as baselineStatusEnum, x as blastRadiusEnum, z as collectionStatusEnum, A as confidenceEnum, E as createAdapterFromConfig, G as createClient, H as createD1Adapter, K as createD1Client, O as createPostgresAdapter, P as createPostgresClient, V as createSimpleAdapter, R as dkimSelectors, Q as domainNotes, S as domainTags, T as domains, U as fieldComparisonStatusEnum, W as findings, X as fleetReportStatusEnum, Y as isDbError, $ as legacyAccessLogs, a0 as legacyToolTypeEnum, a1 as mailEvidence, a2 as mailProviderEnum, a3 as mapDatabaseError, a4 as mismatchReports, a5 as monitoredDomains, a6 as monitoringScheduleEnum, a7 as observations, a8 as parseSSLConfig, a9 as partitionDbResults, aa as probeStatusEnum, ab as probeTypeEnum, ac as providerBaselines, ad as recordSets, ae as remediationPriorityEnum, af as remediationRequests, ag as remediationStatusEnum, ah as resultStateEnum, ai as riskPostureEnum, aj as rulesetVersions, ak as savedFilters, al as selectorConfidenceEnum, am as selectorProvenanceEnum, an as sessions, ao as severityEnum, ap as shadowComparisons, aq as shadowStatusEnum, ar as sharedReportStatusEnum, as as sharedReportVisibilityEnum, at as sharedReports, au as snapshots, av as suggestions, aw as templateOverrides, ax as toNotFoundError, ay as toTenantIsolationError, az as unwrapDbResultOr, aA as users, aB as vantageTypeEnum, aC as zoneManagementEnum } from '../nitro/nitro.mjs';
...
class R {
  ...
  async findByIdResult(e) {
    return ac(() => this.repo.findById(e), "Domain", e);
  }
  async findByNameResult(e, t) {
    const s = await ac(() => t ? this.repo.findByNameAndTenant(e, t) : this.repo.findByName(e), "Domain", e);
    return t && s.isOk() ? dc(s.value, s.value.tenantId, t, "Domain") : s;
  }
  ...
  async createResult(e) {
    ...
    return oc(() => this.repo.create(e), (t) => new ye({ message: t instanceof Error ? t.message : "Failed to create domain", code: "QUERY_FAILED", table: "Domain", operation: "create" }));
  }
  async findOrCreateResult(e) {
    return oc(() => this.repo.findOrCreate(e), (t) => new ye({ message: t instanceof Error ? t.message : "Failed to find or create domain", code: "QUERY_FAILED", table: "Domain", operation: "findOrCreate" }));
  }
  async updateResult(e, t) {
    return await this.repo.findById(e) ? ac(() => this.repo.update(e, t), "Domain", e) : Result.err(ye.notFound("Domain", e));
  }
  async updateForTenantResult(e, t, s) {
    const a = await this.findByIdResult(e);
    if (a.isErr()) return a;
    const o = dc(a.value, a.value.tenantId, s, "Domain");
    return o.isErr() ? o : ac(() => this.repo.update(e, t), "Domain", e);
  }
  async deleteResult(e) {
    return ac(() => this.repo.delete(e), "Domain", e);
  }
  async deleteForTenantResult(e, t) {
    const s = await this.findByIdResult(e);
    if (s.isErr()) return s;
    const a = dc(s.value, s.value.tenantId, t, "Domain");
    return a.isErr() ? a : ac(() => this.repo.delete(e), "Domain", e);
  }
  async searchByNameResult(e, t) {
    return oc(() => this.repo.searchByName(e, t), (s) => new ye({ message: s instanceof Error ? s.message : "Search failed", code: "QUERY_FAILED", table: "Domain", operation: "search" }));
  }
  async findAllResult(e, t) {
    return oc(() => this.repo.findAll(e, t), (s) => new ye({ message: s instanceof Error ? s.message : "Find all failed", code: "QUERY_FAILED", table: "Domain", operation: "findAll" }));
  }
}
function V(p) {
  return new R(p);
...
class E {
  ...
  async findByIdResult(e) {
    return ac(() => this.repo.findById(e), "Snapshot", e);
  }
  async findByDomainResult(e, t) {
    return oc(() => this.repo.findByDomain(e, t), (s) => new ye({ message: s instanceof Error ? s.message : "Failed to find snapshots", code: "QUERY_FAILED", table: "Snapshot", operation: "findByDomain", identifier: e }));
  }
  ...
  async findRecentByDomainResult(e, t) {
    return oc(() => this.repo.findRecentByDomain(e, t), (s) => new ye({ message: s instanceof Error ? s.message : "Failed to check recent snapshot", code: "QUERY_FAILED", table: "Snapshot", operation: "findRecentByDomain", identifier: e }));
  }
  async createResult(e) {
    return e.domainId ? oc(() => this.repo.create(e), (t) => new ye({ message: t instanceof Error ? t.message : "Failed to create snapshot", code: "QUERY_FAILED", table: "Snapshot", operation: "create" })) : Result.err(new ye({ message: "Domain ID is required", code: "CONSTRAINT_VIOLATION", table: "Snapshot", operation: "create" }));
  }
  async updateErrorResult(e, t) {
    const s = await this.findByIdResult(e);
    return s.isErr() ? s : ac(() => this.repo.updateError(e, t), "Snapshot", e);
  }
  async updateDurationResult(e, t) {
    const s = await this.findByIdResult(e);
    return s.isErr() ? s : ac(() => this.repo.updateDuration(e, t), "Snapshot", e);
  }
  async updateRulesetVersionResult(e, t) {
    const s = await this.findByIdResult(e);
    return s.isErr() ? s : ac(() => this.repo.updateRulesetVersion(e, t), "Snapshot", e);
  }
  async listResult(e) {
    return oc(() => this.repo.list(e), (t) => new ye({ message: t instanceof Error ? t.message : "Failed to list snapshots", code: "QUERY_FAILED", table: "Snapshot", operation: "list" }));
  }
  async countByDomainResult(e) {
    return oc(() => this.repo.countByDomain(e), (t) => new ye({ message: t instanceof Error ? t.message : "Failed to count snapshots", code: "QUERY_FAILED", table: "Snapshot", operation: "countByDomain", identifier: e }));
  }
  async findNeedingBackfillResult(e, t) {
    return oc(() => this.repo.findNeedingBackfill(e, t), (s) => new ye({ message: s instanceof Error ? s.message : "Failed to find backfill candidates", code: "QUERY_FAILED", table: "Snapshot", operation: "findNeedingBackfill" }));
  }
}
function j(p) {
  return new E(p);
...
export { ye as DbError, ne as DomainRepository, R as DomainRepositoryResults, q as FleetReportRepository, $ as ProbeObservationRepository, E as SnapshotRepositoryResults, oc as dbResult, ac as dbResultOrNotFound, dc as ensureTenantIsolation, Bo as fleetReports, Lo as probeObservations, V as withDomainResults, j as withSnapshotResults };
//# sourceMappingURL=index-DAbQFoYJ.mjs.map
