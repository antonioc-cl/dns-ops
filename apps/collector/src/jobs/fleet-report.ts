...
async function runChecks(
  ...
  if (checkTypes.includes('spf')) {
    const spfRecords = observations.filter(o => 
      o.queryType === 'TXT' && 
      o.status === 'success' &&
  ...
  if (checkTypes.includes('dmarc')) {
    const dmarcRecords = observations.filter(o =>
      o.queryType === 'TXT' &&
  ...
  if (checkTypes.includes('mx')) {
    const mxRecords = observations.filter(o =>
      o.queryType === 'MX' &&
      o.status === 'success' &&
      o.answerSection &&
    ...
    if (mxRecords.length === 0) {
      ...
      const nullMx = mxRecords[0].answerSection?.find(a => 
        a.data.trim() === '0 .'
      );
  ...
  if (checkTypes.includes('dkim')) {
    const dkimRecords = observations.filter(o =>
      o.queryType === 'TXT' &&
...
function generateSummary(results: FleetReportResult[], checkTypes: string[]): Record<string, unknown> {
