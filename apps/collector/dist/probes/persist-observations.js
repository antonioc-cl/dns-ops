 ...
 */
export function smtpResultToObservation(snapshotId, result) {
    ...
    const probeData = result.certificate
        ? {
 ...
 */
export function mtastsResultToObservation(snapshotId, hostname, result) {
    // Determine status from result
    let status = 'error';
    if (result.success) {
        status = 'success';
 ...
 */
export async function persistProbeObservations(db, snapshotId, _tenantId, observations) {
    if (!db) {
    ...
    try {
        const created = await repo.createMany(observations);
 ...
 */
export async function getProbeObservations(db, snapshotId) {
    if (!db) {
        return [];
    }
    const repo = new ProbeObservationRepository(db);
...
//# sourceMappingURL=persist-observations.js.map