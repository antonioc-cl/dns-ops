...
export function findingsToCheckResults(findings, checkTypes) {
    const results = [];
    ...
    for (const checkType of checkTypes) {
        ...
        const relevantFindings = findings.filter((f) => relevantTypes.some((t) => f.type.startsWith(t.replace('.', '.'))));
        // Also match by prefix for flexibility
        const prefixFindings = findings.filter((f) => {
            if (checkType === 'spf')
                return f.type.includes('spf');
            if (checkType === 'dmarc')
                return f.type.includes('dmarc');
            if (checkType === 'mx')
                return f.type.includes('mx') && f.type.startsWith('mail.');
            if (checkType === 'dkim')
                return f.type.includes('dkim');
            if (checkType === 'infrastructure')
                return f.type.startsWith('dns.auth');
            if (checkType === 'delegation')
                return f.type.includes('delegation') || f.type.includes('ns');
            return false;
        });
 ...
 */
// Export for testing
export function mapSeverityToStatus(severity) {
    switch (severity) {
        case 'critical':
        case 'high':
            return 'fail';
...
// Export for testing
export function generateSummary(results, checkTypes) {
    const summary = {
