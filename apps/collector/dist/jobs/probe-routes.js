...
import { fetchMTASTSPolicy, probeAllowlistManager, probeMXHosts, probeSMTPStarttls, validateMTASTSTxtRecord, } from '../probes/index.js';
...
probeRoutes.post('/mta-sts', async (c) => {
    ...
    const config = getEnvConfig();
    const result = await getProbeSemaphore().run(() => fetchMTASTSPolicy(domain, tenantId, {
...
probeRoutes.post('/smtp-starttls', async (c) => {
    ...
    if (hostname) {
        ...
        const result = await getProbeSemaphore().run(() => probeSMTPStarttls(hostname, tenantId, {
            port: port || 25,
    ...
    if (mxRecords && Array.isArray(mxRecords) && mxRecords.length > 0) {
        ...
        const hosts = mxRecords.map((record) => {
