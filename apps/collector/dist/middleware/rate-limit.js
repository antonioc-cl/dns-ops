 ...
 */
function refillTokens(entry, limit, windowMs) {
    const now = Date.now();
 ...
 */
function tryConsume(entry, _limit) {
 ...
 */
function getRetryAfter(windowMs) {
 ...
 */
function createKey(tenantId, path) {
 ...
 */
export function rateLimitMiddleware(scope) {
    return async (c, next) => {
 ...
 */
export function resetRateLimit(tenantId, path) {
    if (tenantId && path) {
        const key = createKey(tenantId, path);
 ...
 */
export function getRateLimitStatus(tenantId, path) {
 ...
 */
export function checkRateLimit(scope, tenantId, path) {
